...
import com.bomweb.offustransaction.xml.ISO8583offusXML;
...
public class OFFUSTransaction {
	...
	public static Transactionoffus_OUT offusTransaction(Transactionoffus_IN in) {
		Transactionoffus_OUT out = new Transactionoffus_OUT();
		byte[] trnReq = ISO8583offusXML.createXML(in);
		
		OFFUSTransactionXML tranReq = saveOffusRequest(trnReq, in);
		byte[] response = postTransaction(tranReq.getTranReq());
		try {
			if (response != null && response.length > 0) {
				tranReq = saveOffusResponse(tranReq, response);
	...
	private static OFFUSTransactionXML saveOffusRequest(byte[] trnReq, Transactionoffus_IN in) {
	...
	private static OFFUSTransactionXML saveOffusResponse(OFFUSTransactionXML tranReq, byte[] response) {
		Session session = HibernateUtil.getSessionfactory().openSession();
		try {
			File xmlFile = new File("E://BOM/Enrollment/offustxn_rsp" + tranReq.getREQ_rrn() + ".xml");
			if (xmlFile.exists())
				xmlFile.delete();
