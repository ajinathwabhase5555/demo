...
public class BBPSService {
	...
	public static BBPSOUT payBillRequest(BBPSIN in, BBPSOUT out) {
		...
		try {
			if (response != null) {
				...
				out.setErrorDesc(payBill.getErrDesc());
				out.setResponseCode(payBill.getResponseCode());
				out.setResponseDesc(payBill.getResponseDesc());
				if (payBill.getErrCode().equals("0")) {
	...
	private static PayBill savePayBillResponse(PayBill payBill, byte[] response) {
		...
		try {
			...
			if (responseXML.getResponse().getErrorMessages().getErrCode().equals("0")) {
	...
	private static PayBill savePayBillReq(BBPSIN in) {
		...
		try {
			...
			payBill.setPosCode("05");
			payBill.setPosEntryMode("019");
			...
			if (!in.getAdvPayAmt().equals(""))
				payBill.setAdvPayAmt(in.getAdvPayAmt());
			session.beginTransaction();
			session.saveOrUpdate(payBill);
			session.getTransaction().commit();
		} catch (Exception e) {
	...
	private static FetchBill saveFetchBillReq(BBPSIN in) {
		...
		try {
			...
			if (!in.getAadhaarNo().equals(""))
				fetchBill.setAadhaarNo("XXXXXXXX" + in.getAadhaarNo().substring(in.getAadhaarNo().length() - 4));
			if (!in.getVid().equals(""))
				fetchBill.setVid("XXXXXXXX" + in.getVid().substring(in.getVid().length() - 8));
			session.beginTransaction();
