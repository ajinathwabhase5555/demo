...
public class EKycManager {
	Log log = LogFactory.getLog(EKycManager.class);

	public EKycResponse sendEKycRequestS(EKycRequest eKycRequest) throws SocketTimeoutException {
		EKycResponse eKycResponse = new EKycResponse();
		try {
			byte[] request = EKYC_ISO8583.createISO(eKycRequest).getBytes();
			EkycLog ekycLog = saveEkycRequest(request);
			byte[] responseArray = sendRequest(ekycLog.getEkycReq());
	 ...
	 * @param responseArray
	 */
	private EkycLog saveEkycResponse(byte[] ekycResponse, String[] response, int id) {
		Session session = HibernateUtil.getSessionfactory().openSession();
		EkycLog ekycLog = (EkycLog) session.load(EkycLog.class, id);
		try {
			Blob ekycRsp = new SerialBlob(ekycResponse);
			ekycLog.setResponseMsg(ekycRsp);
			ekycLog.setRspProcessingCode(response[3]);
			ekycLog.setRspTransactionDateTime(response[7]);
			ekycLog.setRspStan(response[11]);
			ekycLog.setRspTransactionTime(response[12]);
			ekycLog.setRspTransactionDate(response[13]);
			ekycLog.setRspRRNNo(response[37]);
			ekycLog.setResponseCode(response[39]);
			ekycLog.setRspTerminalID(response[41]);
			ekycLog.setRspCardAcceptorID(response[42]);
			ekycLog.setRspCardAcceptorName(response[43]);
			if (response[39].equals("00")) {
				Blob xmlData = new SerialBlob(response[112].getBytes());
				ekycLog.setXmlData(xmlData);
	 ...
	 * @return
	 */
	private EkycLog saveEkycRequest(byte[] eKycRequest) {
		Session session = HibernateUtil.getSessionfactory().openSession();
		MessageParser parser = new MessageParser();
		EkycLog ekycLog = new EkycLog();
		try {
			String[] request = parser.parseAEPSMsg(eKycRequest, "transaction");
			...
			ekycLog.setAccountNo(request[2]);
			ekycLog.setProcessingCode(request[3]);
			ekycLog.setTransactionDateTime(request[7]);
			ekycLog.setStan(request[11]);
			ekycLog.setTransactionTime(request[12]);
			ekycLog.setTransactionDate(request[13]);
			ekycLog.setMerchantType(request[18]);
			ekycLog.setPosEntryMode(request[22]);
			ekycLog.setPosConditionCode(request[25]);
			ekycLog.setAiic(request[32]);
			ekycLog.setRrnNo(request[37]);
			ekycLog.setTerminalID(request[41]);
			ekycLog.setCardAcceptorID(request[42]);
			ekycLog.setCardAcceptorName(request[43]);
			ekycLog.setAgentsInformation(request[46]);
			ekycLog.setCurrencyCode(request[49]);
			ekycLog.setDatetime(request[73]);
			ekycLog.setVersion(request[100]);
			ekycLog.setConsentType("E-KYC");
			session.beginTransaction();
			session.saveOrUpdate(ekycLog);
			session.getTransaction().commit();
		} catch (Exception e) {
	...
	private EKycResponse setClientResponse(String[] responseArray, EKycResponse eKycResponse) throws Exception {
		...
		EKYCResponseXML ekycResponseXML = (EKYCResponseXML) unmarshaller.unmarshal(xmlFile);
...
//		xmlFile.delete();
	...
	private byte[] sendRequest(byte[] dataSrc) throws IOException {
		...
		try {
			...
			HttpPost httpPost = new HttpPost("http://125.18.108.188:7003/Servlet/IBSOtherVendorRequestServlet"); //OtherVendorEkycServlet
